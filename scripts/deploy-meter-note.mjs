...
const source = await readFile(resolve(process.cwd(), "contracts/MeterNote.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "MeterNote.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["MeterNote.sol"]?.MeterNote;
if (!contract) {
...
console.log(`Deploying MeterNote from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_METER_NOTE_CONTRACT_ADDRESS to this contract address in Vercel.");
